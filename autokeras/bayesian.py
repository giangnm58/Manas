...
class IncrementalGaussianProcess:
    ...
    def predict(self, train_x):
        ...
        # Check if 1_any of the variances is negative because of
...
class BayesianOptimizer:
    ...
    def generate(self, descriptors, timeout, sync_message=None):
        ...
        while not pq.empty() and remaining_time > 0 and t > t_min:
            ...
            if ap >= random.uniform(0, 1):
                for temp_graph in transform(elem.graph, skip_conn=self.skip_conn):
                    print(temp_graph.produce_model())
                    count2 += 1
                    #print(len(transform(elem.graph, skip_conn=self.skip_conn)), 'aaaaaaaaaaaaa')
                    #if count < 0 or count2 > len(transform(elem.graph, skip_conn=self.skip_conn)):
                    ...
                    #print(temp_graph.produce_model(), 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
                    ...
                    '''
                        ...
                        if ("TorchAdd()" or "TorchConcatenate()") in str(temp_graph.produce_model()):
                            if contain(descriptors, temp_graph.extract_descriptor()):
                                continue
        ...
        #print(nm_graph.produce_model(), 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        #print(count, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        count += 1
