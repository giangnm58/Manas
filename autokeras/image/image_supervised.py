...
class ImageSupervised(DeepTaskSupervised, ABC):
    ...
    def fit(self, x, y, num_model=0, time_limit=None):
        ...
        super().fit(x, y, num_model, time_limit)
...
class ImageClassifier(ImageSupervised):
    ...
    def export_autokeras_model(self, model_file_name):
        ...
        """
        if self.checker == 1:
            portable_model = PortableImageClassifier(graph=self.cnn3_10.best_model,
        ...
        '''
        if self.checker == 2:
            portable_model = PortableImageClassifier(graph=self.cnn3_100.best_model,
                                                     y_encoder=self.y_encoder,
                                                     data_transformer=self.data_transformer,
                                                     resize_params=self.resize_shape,
                                                     path=self.path)
        ...
        if self.checker == 3:
            portable_model = PortableImageClassifier(graph=self.cnn3_1000.best_model,
                                                     y_encoder=self.y_encoder,
                                                     data_transformer=self.data_transformer,
                                                     resize_params=self.resize_shape,
                                                     path=self.path)
        '''
        if self.checker == 2:
            portable_model = PortableImageClassifier(graph=self.cnn1_10.best_model,
                                                     y_encoder=self.y_encoder,
                                                     data_transformer=self.data_transformer,
                                                     resize_params=self.resize_shape,
                                                     path=self.path)
        '''
        if self.checker == 5:
            portable_model = PortableImageClassifier(graph=self.cnn1_100.best_model,
                                                     y_encoder=self.y_encoder,
                                                     data_transformer=self.data_transformer,
                                                     resize_params=self.resize_shape,
                                                     path=self.path)
        if self.checker == 6:
            portable_model = PortableImageClassifier(graph=self.cnn1_1000.best_model,
                                                     y_encoder=self.y_encoder,
                                                     data_transformer=self.data_transformer,
                                                     resize_params=self.resize_shape,
                                                     path=self.path)
