...
from autokeras.net_module import CnnModule3_10,  \
    CnnModule1_10, CnnModule
from autokeras.search import BayesianSearcher, train, Searcher
...
class Supervised(ABC):
    ...
    @abstractmethod
    def fit(self, x_train, y_train, num_model, time_limit=None):
...
class DeepTaskSupervised(SearchSupervised):
    ...
    def __init__(self, verbose=False, path=None, resume=False, searcher_args=None,
        ...
        if resume:
            ...
            self.verbose = verbose
            #if .shape[3] == 3 and 1 <= self.get_n_output_node() <= 10:
            #if Searcher.get_best_model_id() == 3:

            self.cnn3_10 = CnnModule3_10(self.loss, self.metric, searcher_args, path, verbose, search_type)
            #self.cnn3_100 = CnnModule3_100(self.loss, self.metric, searcher_args, path, verbose, search_type)
            #self.cnn3_1000 = CnnModule3_1000(self.loss, self.metric, searcher_args, path, verbose, search_type)
            self.cnn1_10 = CnnModule1_10(self.loss, self.metric, searcher_args, path, verbose, search_type)
            #self.cnn1_100 = CnnModule1_100(self.loss, self.metric, searcher_args, path, verbose, search_type)
            #self.cnn1_1000 = CnnModule1_1000(self.loss, self.metric, searcher_args, path, verbose, search_type)

            #self.cnn = CnnModule(self.loss, self.metric, searcher_args, path, verbose, search_type)
    def fit(self, x, y, num_model, time_limit=None):
        """Find the best neural architecture for classifying the training data and train it.
        ...
        if time_limit is None:
            time_limit = 24 * 60 * 60
        #print(self.get_n_output_node(),'aaaaaaaaaa')
        #print(x_train.shape[3],'bvvvvvvvvv')
        if x_train.shape[3] == 3:
            self.checker = 1
            current1 = datetime.now()
            self.cnn3_10.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, num_model, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn3_10.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)
        if x_train.shape[3] == 1:
            self.checker = 2
            current1 = datetime.now()
            self.cnn1_10.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, num_model, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn1_10.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)
        '''
        if x_train.shape[3] == 3 and 1 <= self.get_n_output_node() <= 10:
            self.checker = 1
            current1 = datetime.now()
            self.cnn3_10.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn3_10.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)

        if x_train.shape[3] == 3 and 10 < self.get_n_output_node() <= 100:
            self.checker = 2
            current1 = datetime.now()
            self.cnn3_100.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn3_100.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)

        if x_train.shape[3] == 3 and 100 < self.get_n_output_node():
            self.checker = 3
            current1 = datetime.now()
            self.cnn3_1000.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn3_1000.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)

        if x_train.shape[3] == 1 and 1 <= self.get_n_output_node() <= 10:
            self.checker = 4
            current1 = datetime.now()
            self.cnn1_10.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn1_10.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)

        if x_train.shape[3] == 1 and 10 < self.get_n_output_node() <= 100:
            self.checker = 5
            current1 = datetime.now()
            self.cnn1_100.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)

            self.cnn1_100.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)
        if x_train.shape[3] == 1 and 100 < self.get_n_output_node():
            self.checker = 6
            current1 = datetime.now()
            self.cnn1_1000.fit_temp(self.get_n_output_node(), x_train.shape, train_data, valid_data, self.checker)
            current2 = datetime.now()
            print('Total Model Searching Time: ', current2 - current1)
            self.cnn1_1000.fit(self.get_n_output_node(), x_train.shape, train_data, valid_data, time_limit)
        '''
    def final_fit(self, x_train, y_train, x_test, y_test, id = None, path = None, trainer_args=None, retrain=False):
        ...
        if trainer_args is None:
            trainer_args = {'max_no_improvement_num': 60}
        ...
        '''
        if x_train.shape[3] == 3 and 1 <= self.get_n_output_node() <= 10:
            self.cnn3_10.final_fit(train_data, test_data, trainer_args, retrain)
        if x_train.shape[3] == 3 and 10 < self.get_n_output_node() <= 100:
            self.cnn3_100.final_fit(train_data, test_data, trainer_args, retrain)
        if x_train.shape[3] == 3 and 100 < self.get_n_output_node():
            self.cnn3_1000.final_fit(train_data, test_data, trainer_args, retrain)
        if x_train.shape[3] == 1 and 1 <= self.get_n_output_node() <= 10:
            self.cnn1_10.final_fit(train_data, test_data, trainer_args, retrain)
        if x_train.shape[3] == 1 and 10 < self.get_n_output_node() <= 100:
            self.cnn1_100.final_fit(train_data, test_data, trainer_args, retrain)
        if x_train.shape[3] == 1 and 100 < self.get_n_output_node():
            self.cnn1_1000.final_fit(train_data, test_data, trainer_args, retrain)
        '''
        if x_train.shape[3] == 3:
            self.cnn3_10.final_fit(train_data, test_data, id, path, trainer_args, retrain)
        if x_train.shape[3] == 1:
            self.cnn1_10.final_fit(train_data, test_data, id, path, trainer_args, retrain)
    @property
    @abstractmethod
    def metric(self):
        pass
    ...
    def export_keras_model(self, model_file_name):
        ...
        """
        if self.checker == 1:
            self.cnn3_10.best_model.produce_keras_model().save(model_file_name)
        '''
        if self.checker == 2:
            self.cnn3_100.best_model.produce_keras_model().save(model_file_name)
        if self.checker == 3:
            self.cnn3_1000.best_model.produce_keras_model().save(model_file_name)
        '''
        if self.checker == 2:
            self.cnn1_10.best_model.produce_keras_model().save(model_file_name)
        '''
        if self.checker == 5:
            self.cnn1_100.best_model.produce_keras_model().save(model_file_name)
        if self.checker == 6:
            self.cnn1_1000.best_model.produce_keras_model().save(model_file_name)
        '''
    ...
    def predict(self, x_test):
        ...
        """
        x_test = self.preprocess(x_test)
        #print(x_test,'cccccccccccccc')
        test_loader = self.data_transformer.transform_test(x_test)
        if x_test.shape[3] == 3:
            return self.inverse_transform_y(self.cnn3_10.predict(test_loader))
        if x_test.shape[3] == 1:
            return self.inverse_transform_y(self.cnn1_10.predict(test_loader))

        '''
        if x_test.shape[3] == 3 and 1 <= self.get_n_output_node() <= 10:
            return self.inverse_transform_y(self.cnn3_10.predict(test_loader))
        if x_test.shape[3] == 3 and 10 < self.get_n_output_node() <= 100:
            return self.inverse_transform_y(self.cnn3_100.predict(test_loader))
        if x_test.shape[3] == 3 and 100 < self.get_n_output_node():
            return self.inverse_transform_y(self.cnn3_1000.predict(test_loader))
        if x_test.shape[3] == 1 and 1 <= self.get_n_output_node() <= 10:
            return self.inverse_transform_y(self.cnn1_10.predict(test_loader))
        if x_test.shape[3] == 1 and 10 < self.get_n_output_node() <= 100:
            return self.inverse_transform_y(self.cnn1_100.predict(test_loader))
        if x_test.shape[3] == 1 and 100 < self.get_n_output_node():
            return self.inverse_transform_y(self.cnn3_1000.predict(test_loader))
        '''
    def evaluate(self, x_test, y_test):
        """Return the accuracy score between predict value and `y_test`.
