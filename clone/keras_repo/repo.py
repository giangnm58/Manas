...
for repo in list:
    ...
    if 10000 >= count >= 0:
        ...
        name = repo[0][19:]
        path = string_extraction(name)
        print(path)
        model_files = open('repo_name2.txt', "a", encoding="ISO-8859-1")

        os.system("git clone https://github.com/" + name)
        for r, d, f in os.walk(path):
            for file in f:
                if '.py' in file:
                    k = open(os.path.join(r, file), encoding="ISO-8859-1")
                    temp_file = k.read()
                    if 'keras'.lower() in temp_file.lower():
                        print(os.path.join(r, file))
                        model_files.write(repo[0][0:(len(repo[0])-4)] + "/tree/master/" + string_extraction2(os.path.join(r, file)) + '\n')
